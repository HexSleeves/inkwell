...
- Pragmatic write rate limiting (CIL-128): a process-wide GCRA limiter (`governor`) throttles mutations (`POST`/`PUT`/`PATCH`/`DELETE`) + `/ask`, bucketed by authenticated credential (else client IP), configurable via `INKWELL_WRITE_RATE_LIMIT` req/min (default 60; `0` disables); over-limit → `429` + `Retry-After`. Reads + public HTML never throttled. Middleware in `src/http/rate_limit.rs`.
