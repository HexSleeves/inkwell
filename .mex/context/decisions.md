...
**Date:** 2026-06-23
**Status:** Active
**Decision:** Mutations enforce a scope — `write` for create/update/delete, `publish` for publish/unpublish (`require_scope`) — and, for non-admin principals, ownership: `authorize_mutation` compares `documents.owner_id` to `principal.author_id` and returns 403 on mismatch; admin (the shared/MCP key) bypasses ownership. `create` stamps `owner_id` from the principal (DB falls back to bootstrap admin when absent). Draft READ visibility now requires the `read` scope (`can_see_drafts`); admin implies all scopes. Per-owner draft read ISOLATION is **deferred to slice 3b**.
**Reasoning:** The write/publish ownership boundary is the real privilege win — a leaked `write` token cannot touch others' notes or escalate. Read-isolation (an author seeing only their own drafts) needs the binary `Visibility` (Public/All) reworked into an owner-aware filter threaded through ~6 query modules (documents/links/chunks/search/webmentions/pages) — a disproportionate change for today's effectively single-author+admin garden, so it is its own slice. Ownership uses a dedicated `get_document_owner` query rather than adding `owner_id` to every `Document` SELECT (one cheap query per mutation; far less blast radius).
