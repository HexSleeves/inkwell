...
pub async fn feed(State(state): State<AppState>, headers: HeaderMap) -> Response {
    ...
    let site_title = &state.config.site_title;
    let updated = documents
        .first()
        .map(|doc| crate::domain::document::timestamp::serialize_to_string(&doc.updated_at))
        .unwrap_or_else(|| "1970-01-01T00:00:00.000Z".to_string());
    // Optional feed-level author element when INKWELL_SITE_AUTHOR is set.
    let author_element = state
        .config
        .site_author
        .as_deref()
        .map(|name| format!("  <author><name>{}</name></author>\n", escape_xml(name)))
        .unwrap_or_default();
    // Optional subtitle from INKWELL_SITE_DESCRIPTION.
    let subtitle_element = state
        .config
        .site_description
        .as_deref()
        .map(|desc| format!("  <subtitle>{}</subtitle>\n", escape_xml(desc)))
        .unwrap_or_default();
